...
.add-banner-card {
    ...
    width: 100%;
    height: 250px;
    border: 2px dashed #01073c;
    border-radius: 10px;
    background: linear-gradient(135deg, #eef5ff, #d0e4ff);
    transition: all 0.3s ease-in-out;
    text-align: center;
}
.upcoming-video-card.add-video-card {
    display: flex;
    justify-content: center;
    align-items: center;
    width: 100%;
...
.add-icon ion-icon {
    font-size: 28px;
    color: linear-gradient(135deg, #01073c, #040d66);
   
}

.add-video-btn:hover .add-icon {
    background: linear-gradient(135deg, #01073c, #040d66);
}
...
.add-video-btn:hover {
    color: linear-gradient(135deg, #040d66, #01073c);
}
                        ...
                        <div class="videos-container">
                            <div class="add-banner-card">
                                <a href="#" class="add-video-btn" id="open-add-modal">
                                    <span class="add-icon" ><ion-icon name="add-outline"></ion-icon></span>
                                    <span class="add-text">Add New Movie/series</span>
                                </a>
                            </div>
    ...
    <div class="modal" id="add-modal">
      <div class="modal-content">
          ...
          <form id="add-form">
              ...
              <div class="form-group">
                  <label class="input-label">Genere Type</label>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Handle form submission to add new movie
    document.getElementById('add-form').addEventListener('submit', function (e) {
        e.preventDefault(); // Prevent the default form submission

        // Get form data
        const imageInput = document.getElementById('image');
        const genreSelect = document.querySelector('.video-select');
        const uploadLinkInput = document.getElementById('name');

        const imageFile = imageInput.files[0];
        const genre = genreSelect.value;
        const uploadLink = uploadLinkInput.value;

        // Create a new video-card element
        const newVideoCard = document.createElement('div');
        newVideoCard.classList.add('video-card', 'banner-card');

        // Create the video-image div
        const videoImageDiv = document.createElement('div');
        videoImageDiv.classList.add('video-image');

        // Create the video-card-overlay div
        const videoCardOverlayDiv = document.createElement('div');
        videoCardOverlayDiv.classList.add('video-card-overlay');

        // Create the video-thumbnail img
        const videoThumbnailImg = document.createElement('img');
        videoThumbnailImg.src = URL.createObjectURL(imageFile); // Use the uploaded image
        videoThumbnailImg.alt = 'Thumbnail';
        videoThumbnailImg.classList.add('video-thumbnail');

        // Append the img to video-image div
        videoImageDiv.appendChild(videoCardOverlayDiv);
        videoImageDiv.appendChild(videoThumbnailImg);

        // Create the video-info div
        const videoInfoDiv = document.createElement('div');
        videoInfoDiv.classList.add('video-info');

        // Create the toggle-banner-btn button
        const toggleBannerBtn = document.createElement('button');
        toggleBannerBtn.classList.add('toggle-banner-btn');
        toggleBannerBtn.textContent = 'Show';
        toggleBannerBtn.href = uploadLink; // Set the upload link

        // Append the button to video-info div
        videoInfoDiv.appendChild(toggleBannerBtn);

        // Append the video-image and video-info to newVideoCard
        newVideoCard.appendChild(videoImageDiv);
        newVideoCard.appendChild(videoInfoDiv);

        // Insert the new video-card after the add-banner-card
        const addBannerCard = document.querySelector('.add-banner-card');
        const videosContainer = document.querySelector('.videos-container');
        videosContainer.insertBefore(newVideoCard, addBannerCard.nextSibling);
